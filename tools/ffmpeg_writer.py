...
def ffmpeg_write_video(clip, filename, fps, codec="libx264", bitrate=None,
    ...
    with FFMPEG_VideoWriter(filename, clip.size, fps, codec = codec,
        ...
        clip_iter_frames = clip.iter_frames(progress_bar=progress_bar, with_times=True,
                         fps=fps, dtype="uint8")
        index = 0
        for t,frame in clip_iter_frames:
            if index%100==0:
                info_log("\r视频生成进度: {:.2%}".format(index/len(clip_iter_frames)))
